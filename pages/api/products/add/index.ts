...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    if (req.method === 'POST') {
        try {
            const { title, price, image, altImage, category, description, quantity } = req.body;
            ...
            if (!shopifyResponse.ok) {
                ...
                return res.status(shopifyResponse.status).json({error: `Error creating product: ${shopifyResponse.statusText}`});
            }

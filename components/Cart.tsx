        ...
        <div className={`cart ${cartPage ? `sideCart cartPage` : `sideCart`}`}>
            ...
            <div className={`cartBottom center flex justifyContentCenter`}>
                <div className={`cartButtons toggleButtons w85I m0auto`}>
                    {!cartPage && <button onClick={(e) => router.push(`/cart`)} className={`cartButton productButton whiteBG blackText hoverWhiteText`}>
                        <i title={`Cart | Creative Workshop`} className={`addToCartIcon pink fas fa-shopping-cart`}></i>
                        Cart
                    </button>}
                    <button onClick={(e) => onCheckout(e)} className={`cartButton productButton whiteBG blackText hoverWhiteText ${cartPage ? `w100 center m75t` : ``}`}>
